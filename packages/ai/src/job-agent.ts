...
export async function scoreJobFit(
  ...
  const raw = await createClaudeMessage({
    system: `Score job fit 0-100. Output ONLY JSON: {"score": number, "reason": "one sentence"}. Be realistic. Penalize senior/management titles unless company clearly matches candidate domains (maritime, offshore, ML, etc.).\n${JOB_APPLICATION_GUARDRAILS}`,
    messages: [
      {
        role: 'user',
        content: `Profile summary: ${safe.summary}
Domains: ${safe.domains.join(', ')}
Evidence count: ${safe.evidence.length}
${job.experienceNote ? `\nCandidate level:\n${job.experienceNote}` : ''}

Job: ${job.company} · ${job.role}
Type: ${job.applicationType ?? 'job_posting'}
...
export async function generateApplicationPack(
  ...
  const evidenceJson = JSON.stringify(safe.evidence.slice(0, 40))
  ...
  try {
    ...
    selectedIds = safe.evidence.slice(0, 5).map((e) => e.id)
  }

  const evidenceUsed = safe.evidence.filter((e) => selectedIds.includes(e.id))
  const evidenceBlock = evidenceUsed.map((e) => `- [${e.domain}] ${e.claim}`).join('\n')

  const remoteNote =
    preferences?.remoteRequired || safe.constraints.remotePreferred
      ? 'Candidate strongly prefers remote work. If onsite/hybrid, state remote-only clearly and professionally.'
      : ''

  const profileSummary = variantSummary?.trim() || safe.summary
  const levelNote = job.experienceNote?.trim() ?? ''
  const openApply =
    job.applicationType === 'open_application'
      ? 'This is an OPEN / speculative application to the company (not a single job ID). Write a concise company-focused letter proposing value in their domain.'
      : ''

  const draftCv = await createClaudeMessage({
    system: `Tailor a 1-page CV in markdown. ${localeHint(locale)} Use ONLY provided evidence. Remove irrelevant disciplines. Max 6 bullets per section. ${levelNote}\n${JOB_APPLICATION_GUARDRAILS}`,
    messages: [
      {
        role: 'user',
  ...
  const draftLetter = await createClaudeMessage({
    system: `Draft cover letter (max 320 words). ${localeHint(locale)} Factual only. No "I am excited", "passionate", "dynamic team". ${remoteNote} ${openApply} ${levelNote}\n${JOB_APPLICATION_GUARDRAILS}`,
    messages: [
      {
        role: 'user',
        content: `Role: ${job.role} at ${job.company}\n\nEvidence:\n${evidenceBlock}\n\nConstraints: ${JSON.stringify(safe.constraints)}\n\nJob:\n${job.jobDescription.slice(0, 5000)}`,
      },
    ],
    maxTokens: 2048,
    temperature: 0.35,
  ...
  const voiceHint = safe.voiceSamples.length
    ? `Match this voice style:\n${safe.voiceSamples.join('\n')}`
  ...
  const humanized = await createClaudeMessage({
    ...
    messages: [
      {
        role: 'user',
        content: `Draft to humanize:\n${draftLetter}\n\nRemote preference: ${safe.constraints.remotePreferred}`,
      },
    ],
    maxTokens: 2048,
    temperature: 0.5,
  ...
  return {
    tailoredCv: sanitizeApplicationText(draftCv),
    coverLetter: sanitizeApplicationText(humanized),
    evidenceUsed,
  }
}
...
export async function buildProfileVariants(
  ...
  const safe = profileForApplications(profile)
  const raw = await createClaudeMessage({
    system: `Create three profile variants from one master profile. Output ONLY valid JSON with keys research, industry, gig. Each value: {label, summary (2-3 sentences), emphasis (string array)}. No invented facts.\n${JOB_APPLICATION_GUARDRAILS}`,
    messages: [
      {
        role: 'user',
        content: `Master profile:\n${JSON.stringify({ summary: safe.summary, domains: safe.domains, evidence: safe.evidence.slice(0, 15) })}`,
      },
    ],
    maxTokens: 2048,
    temperature: 0.25,
  ...
  try {
    const parsed = JSON.parse(raw) as Record<string, { label: string; summary: string; emphasis: string[] }>
    for (const key of Object.keys(parsed)) {
      parsed[key]!.summary = sanitizeApplicationText(parsed[key]!.summary)
    }
    return parsed
  } catch {
    return {
      research: { label: 'Research', summary: safe.summary, emphasis: safe.domains },
      industry: { label: 'Industry', summary: safe.summary, emphasis: safe.domains },
      gig: { label: 'Gig / contract', summary: safe.summary, emphasis: ['flexible', 'remote'] },
    }
  }
}
...
export async function generateEmailDraft(
  ...
  const safe = profileForApplications(profile)
  return sanitizeApplicationText(
    await createClaudeMessage({
    system: `Write a short professional application email (max 180 words). Include subject line on first line as "Subject: ...". No AI clichés. Factual only.\n${JOB_APPLICATION_GUARDRAILS}`,
    messages: [
      {
        role: 'user',
        content: `Applicant summary: ${safe.summary.slice(0, 500)}
...
${JOB_APPLICATION_GUARDRAILS}`,
    messages: [
      {
        role: 'user',
        content: `Applicant summary: ${safe.summary.slice(0, 600)}
Domains: ${safe.domains.join(', ')}
...
Applicant email: ${userEmail ?? 'applicant'}`,
  ...
  try {
    ...
    return {
      subject: sanitizeApplicationText(parsed.subject?.trim() || `Following up on ${job.role} at ${job.company}`),
      body: sanitizeApplicationText(parsed.body?.trim() || raw),
    }
  } catch {
    return {
      subject: `Following up on ${job.role} at ${job.company}`,
      body: sanitizeApplicationText(raw),
    }
  }
}
